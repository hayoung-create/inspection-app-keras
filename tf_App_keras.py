...
# ── 1. 페이지 설정 ─────────────────────────────────────────────────────────
st.set_page_config(
    page_title="제품 불량 검사 시스템",
    page_icon="🔍",
    layout="centered"
)

st.title("🔍 가죽 제품 불량 검사 AI")
st.caption("업로드된 이미지를 기반으로 VGG16 모델이 정상 및 불량 여부를 판단합니다.")
...
# ── 2. 모델 로드 (Streamlit 캐싱 적용) ────────────────────────────────────
# @st.cache_resource를 사용하여 앱이 다시 실행되어도 모델을 매번 새로 로드하지 않습니다.
@st.cache_resource
def load_model():
...
if pil_img is not None:
    st.image(pil_img, caption="입력된 이미지", use_container_width=True)

    # ── 4. 검사 실행 버튼 ──────────────────────────────────────────────────
    if st.button("검사 시작", type="primary", use_container_width=True):
        if model is None:
            st.error("모델이 로드되지 않아 검사를 진행할 수 없습니다.")
        else:
            ...
            # ── 5. 결과 표시 ───────────────────────────────────────────────
            # 정상이면 성공(success), 불량이면 에러(error) 메시지 표시
            if label == "정상":
